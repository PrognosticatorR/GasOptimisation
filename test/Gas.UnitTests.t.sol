...
contract GasTest is Test {
    ...
    function test_whitelistTransfer(address _recipient, address _sender, uint256 _amount, string calldata _name, uint256 _tier) public {
    ...
    function test_whitelistEvents(address _recipient, address _sender, uint256 _amount, string calldata _name, uint256 _tier) public {
        _amount = bound(_amount, 0, gas.balanceOf(owner));
        vm.assume(_amount > 3);
        vm.assume(bytes(_name).length < 9);
        _tier = bound(_tier, 1, 244);
    ...
    function testWhiteTranferAmountUpdate(address _recipient, address _sender, uint256 _amount, string calldata _name, uint256 _tier) public {
